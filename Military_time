...
import re
u_time=re.split('\W+', str(input()))
if len(str(int(u_time[0]))) < 2 and "AM" in u_time:
    print('0{}:{}'.format(u_time[0],u_time[1]))
elif int(u_time[0])==12 and "AM" in u_time:
    print('{}:{}'.format((int(u_time[0])-12), u_time[1]))
elif int(u_time[0])==12 and "PM" in u_time:
    print('{}:{}'.format(u_time[0],u_time[1]))
elif "AM" in u_time:
    print('{}:{}'.format(u_time[0],u_time[1]))
else:
    print('{}:{}'.format(str((int(u_time[0]))+12), u_time[1]))
